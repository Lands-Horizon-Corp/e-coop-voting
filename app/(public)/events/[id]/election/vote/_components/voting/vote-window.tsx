  ...
  const { data, castVote, isCasting } = useCastVote(election, (voter) => {
    try {
      ...
      election.positions.forEach((pos) => {
        ...
        const votedCandidates = votes.filter(
          (votedCandidate) => votedCandidate.positionId === id,
        );
      ...
      router.push(
        `/events/${election.eventId}/election/vote/complete?pb=${voter.passbookNumber}&fullname=${`${voter.firstName} ${voter.lastName}`}&picture=${voter.picture}&votes=${voteB64}`,
      );
    } catch (e) {
      router.push(
        `/events/${election.eventId}/election/vote/complete?pb=${voter.passbookNumber}&fullname=${`${voter.firstName} ${voter.lastName}`}&picture=${voter.picture}`,
      );
    }
  });
