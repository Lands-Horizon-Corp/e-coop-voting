...
const CandidateTable = ({ data, positions, params }: Props) => {
  ...
  const { data: Members, refetch } = FilteredEventMembersForCandidateSelection(
    params.id,
    params.electionId,
  );
    ...
    <div className="flex flex-1 flex-col gap-y-2 ">
      <Card className="flex flex-wrap items-center dark:bg-transparent border-0 justify-between p-3 rounded-xl gap-y-2 ">
        ...
        <div className="flex items-center gap-x-2 md:gap-x-4">
          <DataTableViewOptions table={table} storageKey="data-table-view-4" />
          <Button
            onClick={() => {
      ...
      <DataTable className="flex-1 bg-background/50 rounded-" table={table} />
