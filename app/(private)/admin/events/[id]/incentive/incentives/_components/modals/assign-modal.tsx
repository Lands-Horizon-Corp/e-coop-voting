...
const AssignModal = ({ state, onClose, eventId, incentive }: Props) => {
  ...
  const { usersWithAssigned, isFetchingUser, isLoadingUser } =
    userWithAssignedIncentives(eventId);
  const { created, createAssignee, isCreatingAssignee, isError, error } =
    useCreateAssignIncentive(eventId, incentive.id, (data) => onClose(false));
                    ...
                    <PopoverTrigger asChild>
                      <FormControl>
                        <Button
                          variant="outline"
                          role="combobox"
                          disabled={isFetchingUser || isLoadingUser}
                          className={cn(
                            "justify-between",
                    ...
                    {!isLoadingUser && !isFetchingUser && (
                      <PopoverContent className="w-full border-none bg-background/40 backdrop-blur-sm p-0">
                        <Command>
                          ...
                          {/* ADD CommandList HERE with a max-height and overflow-y-auto */}
                          <CommandList className="max-h-[300px] overflow-y-auto custom-scrollbar">
                            <CommandEmpty>User not found.</CommandEmpty>
                            <CommandGroup>
                              {usersWithAssigned.map((user) => {
                                const assigned =
                                  user.assignedIncentive.find(
                                    (assign) =>
                                      assign.incentiveId === incentive.id,
                                  ) !== undefined;
