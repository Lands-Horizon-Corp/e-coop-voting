...
export const assignedQuantitySchema = z.coerce.number({
  invalid_type_error: "invalid alotted",
  required_error: "alotted is required",
});

export const claimRequirementsEnum = z.nativeEnum(ClaimRequirements, {
  invalid_type_error: "invalid incentive requirement type",
});
...
export const createIncentiveSchema = z.object({
  itemName: z
    .string({
      ...
      invalid_type_error: "invalid type for item name",
    })
...
// for claimings
export const claimIdSchema = z.coerce.number({
  invalid_type_error: "claim id is invalid",
...
export const claimsEntrySchema = z.object({
  ...
  assignedId: z.coerce.number({
    invalid_type_error: "invalid assigned id",
...
export const createPublicClaimAuthorizationSchema = z
  .object({
    passbookNumber: passbookNumberSchema,
    otp: otpSchema.optional(),
    birthday: z.coerce.date().optional(),
  })
  .refine((data) => data.otp || data.birthday, {
    message: "Either OTP or birthday must be provided",
    path: ["otp", "birthday"],
  });
